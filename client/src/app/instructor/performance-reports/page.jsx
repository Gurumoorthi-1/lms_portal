...
const Card = ({ children, title, icon: Icon, className = "" }) => (
  ...
  >
    <div className="flex items-center justify-between mb-6">
      ...
      {Icon && <Icon className="text-[#7C3AED]" size={20} />}
    </div>
    {children}
  </motion.div>
);
        ...
        return (
          <div className="flex flex-col gap-1.5 max-w-[220px]">
            ...
            <span className={`inline-flex items-center w-fit px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${
              ...
              matrixCategory === 'Needs Training but Reliable' ? 'bg-blue-500 text-white' : 'bg-slate-100 text-slate-500'
            }`}>
              {matrixCategory}
            </span>
        ...
        return (
          <div className="flex items-center gap-3">
            <button 
              ...
              className="px-4 py-2 bg-purple-50 text-[#7C3AED] rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-[#7C3AED] hover:text-white transition-all flex items-center gap-2 border border-purple-100"
            >
              <FileText size={14} />
              View Analysis
            </button>
                ...
                <div className="space-y-10">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="p-5 bg-purple-50/50 rounded-3xl border border-purple-100">
                        <h4 className="font-black text-slate-900 text-[10px] uppercase tracking-widest mb-2 flex items-center gap-2">
                    ...
                    <div className="space-y-4">
                      {selectedAnalysis.actionableRoadmap?.map((item, idx) => (
                        <div key={idx} className="flex items-start gap-4 p-5 bg-white border border-slate-100 rounded-[32px] shadow-sm">
                          <div className="w-10 h-10 bg-[#7C3AED] text-white rounded-full flex items-center justify-center flex-shrink-0 font-black text-sm">
                            {idx + 1}
                          </div>
                          <div>
                            <div className="flex items-center gap-3 mb-1">
                              <h5 className="font-black text-slate-900 text-sm">{item.task}</h5>
                              <span className="px-2 py-0.5 bg-purple-100 text-[#7C3AED] rounded text-[8px] font-black uppercase tracking-widest">{item.timeframe}</span>
                            </div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-6 py-3 bg-[#7C3AED] text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-lg shadow-purple-500/20 hover:bg-purple-600 transition-all active:scale-95"
            >
              <Download size={16} />
              Consolidated PDF
            </button>
            ...
            <div className="h-80 w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data?.summary?.topPerformers || []}>
                  ...
                  />
                  <Bar dataKey="scores.overall" fill="#7C3AED" radius={[6, 6, 0, 0]} barSize={40} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </Card>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-8">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full h-12 pl-12 pr-4 bg-slate-50 border border-slate-100 rounded-2xl focus:outline-none focus:border-purple-500 transition-all text-sm font-medium"
              />
            </div>
          </div>
                ...
                {table.getHeaderGroups().map(headerGroup => (
                  <tr key={headerGroup.id}>
                    {headerGroup.headers.map(header => (
                      ...
                      >
                        <div className="flex items-center gap-1 cursor-pointer hover:text-[#7C3AED] transition-colors">
