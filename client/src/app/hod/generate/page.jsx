...
const QuestionRow = ({ question, index }) => (
  <motion.div
    ...
    className="flex items-start gap-4 p-5 bg-white border border-[#E2E8F0] shadow-sm rounded-2xl hover:border-[#F97316]/40 transition-colors group"
  >
    <div className="w-9 h-9 bg-slate-50 border border-slate-100 rounded-lg flex items-center justify-center text-xs font-black text-slate-500 shrink-0 group-hover:bg-[#F97316] group-hover:text-white group-hover:border-[#F97316] transition-all">
        ...
        <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase border ${
          question.difficulty === 'Hard' || question.difficulty === 'Advanced'
            ...
            : question.difficulty === 'Intermediate' || question.difficulty === 'Medium'
              ...
              : 'bg-blue-50 text-orange-600 border-blue-100'
        }`}>
          {question.difficulty}
        </span>
...
export default function GenerateExam() {
  ...
  if (loading) return <HodLayout><GenerateSkeleton /></HodLayout>;
      ...
      <div className="max-w-4xl mx-auto space-y-10 pb-20">
        <div>
          <h1 className="text-3xl font-black tracking-tight text-slate-900 flex items-center gap-3">
            <span className="w-10 h-10 bg-[#F97316] rounded-xl flex items-center justify-center shadow-lg shadow-orange-500/20">
        ...
        <div className="bg-white border border-[#E2E8F0] shadow-sm rounded-3xl p-6 md:p-8 space-y-8 relative overflow-hidden">
              ...
              return (
                <button
                  ...
                  className={`flex-1 flex items-center justify-center gap-2 h-12 rounded-xl text-sm font-bold transition-all ${
                    isActive ? 'bg-white text-[#F97316] shadow-sm border border-[#E2E8F0]' : 'text-slate-500 hover:text-[#F97316]'
                  }`}
                >
                  <Icon size={18} /> 
                   ...
                   <div className="flex items-center justify-between mb-4">
                      ...
                      <button
                        ...
                        className={`relative w-12 h-7 rounded-full transition-colors duration-300 ${
                          showMncFilter ? 'bg-orange-600' : 'bg-slate-300'
                        }`}
                      >
                              ...
                              return (
                                <label
                                  ...
                                  className={`flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl border cursor-pointer transition-all text-xs font-bold ${
                                    isChecked
                                      ? 'bg-orange-50 border-purple-300 text-purple-700'
                                      : 'bg-slate-50 border-slate-100 text-slate-600 hover:border-purple-200 hover:bg-orange-50/50'
                                  }`}
                                >
                                  <input
                                    type="checkbox"
                ...
                <div className="pt-4 border-t border-slate-50">
                   <div className="flex items-center justify-between mb-6">
                      ...
                      <button 
                        ...
                        className={`w-14 h-8 rounded-full transition-all relative ${isAptitude ? 'bg-[#F97316]' : 'bg-slate-200'}`}
                      >
                         <motion.div 
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                              ...
                              >
                                 <div className={`w-4 h-4 rounded border-2 flex items-center justify-center ${
                                   aptitudeScenario.length === APTITUDE_SCENARIOS.length ? 'border-[#F97316] bg-[#F97316]' : 'border-slate-300'
                                 }`}>
                ...
                {DIFFICULTIES.map(({ label, icon: Icon }) => (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all gap-2 ${
                      difficulty === label ? 'border-[#F97316] bg-orange-50 text-[#F97316]' : 'border-slate-100 bg-slate-50 text-slate-500 hover:border-slate-200'
                    }`}
                  >
                    <Icon size={20} />
                  ...
                  {QUESTION_COUNTS.map((count) => (
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all ${
                        questionCount === count ? 'border-[#F97316] bg-orange-50 text-[#F97316]' : 'border-slate-100 bg-slate-50 text-slate-500 hover:border-slate-200'
                      }`}
                    >
          ...
          {status !== 'done' ? (
            <button
              ...
              className="w-full h-16 bg-[#F97316] hover:bg-[#EA580C] disabled:opacity-50 text-white rounded-2xl font-black text-lg flex items-center justify-center gap-3 shadow-xl shadow-orange-500/20 hover:-translate-y-0.5 active:scale-[0.98] transition-all mt-4"
            >
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2 flex-1">
                    ...
                    <div>
                      <div className={`text-xs font-black uppercase tracking-wider ${status === 'scraping' ? 'text-[#F97316]' : 'text-emerald-600'}`}>
                  ...
                  {/* Connector */}
                  <div className={`h-0.5 w-12 rounded-full transition-all ${status === 'generating' ? 'bg-[#F97316]' : 'bg-slate-200'}`} />
                  ...
                  <div className="flex items-center gap-2 flex-1">
                    ...
                    <div>
                      <div className={`text-xs font-black uppercase tracking-wider ${status === 'generating' ? 'text-[#F97316]' : 'text-slate-400'}`}>
