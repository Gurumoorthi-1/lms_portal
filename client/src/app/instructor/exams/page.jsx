...
function ExamBoardCard({ exam, index, onDelete }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white border border-[#E2E8F0] shadow-sm rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-5 hover:border-[#7C3AED]/30 transition-all group"
    >
      <div className="flex items-center gap-5">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            <h3 className="font-bold text-slate-900 group-hover:text-[#7C3AED] transition-colors">{exam.title}</h3>
            {exam.isAI && (
              <span className="text-[9px] font-black px-1.5 py-0.5 bg-purple-50 text-[#7C3AED] rounded uppercase tracking-widest border border-purple-100">
                AI GEN
              </span>
            )}
          </div>
          <div className="flex items-center gap-4 text-xs font-bold text-slate-400 flex-wrap">
            ...
            <span className="px-2 py-0.5 bg-slate-50 border border-slate-100 rounded text-[10px] uppercase tracking-wider text-[#7C3AED]">{exam.topic}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between sm:justify-end gap-6">
        ...
        <div className="flex items-center gap-2">
            <button 
              onClick={() => navigate(`/instructor/analytics?examId=${exam._id}`)}
              className="h-10 w-10 bg-slate-50 border border-slate-100 text-slate-400 rounded-xl flex items-center justify-center hover:text-[#7C3AED] hover:border-[#7C3AED] transition-all"
            >
                <Eye size={18} />
            </button>
            <button 
...
export default function InstructorExams() {
  ...
  if (loading) return <InstructorLayout><RepositorySkeleton /></InstructorLayout>;
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <Link to="/instructor/generate">
              <button className="h-12 px-6 bg-[#7C3AED] text-white rounded-2xl font-bold hover:bg-[#6D28D9] transition-all flex items-center gap-2 shadow-lg shadow-purple-500/20">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <div className="flex-1 w-full relative">
            ...
            <input
              ...
              className="w-full h-12 pl-11 pr-4 bg-white border border-[#E2E8F0] rounded-2xl outline-none focus:border-[#7C3AED] text-sm font-medium text-slate-800 transition-all shadow-sm"
            />
          </div>
            ...
            {STATUS_FILTERS.map((f) => (
              <button
                ...
                className={`px-5 py-2.5 text-xs font-black uppercase tracking-tighter rounded-xl transition-all ${
                  filter === f ? 'bg-[#7C3AED] text-white shadow-md' : 'text-slate-500 hover:text-[#7C3AED]'
                }`}
              >
                {f}
              </button>
