...
export default function HodLayout({ children }) {
  ...
  const { user = { username: 'HOD Portal' } } = useUser({ requireAuth: true, redirectIfNoAuth: true });

  useEffect(() => {
    if (!user) return;
    const userId = user.id || user._id;
    if (userId) {
      socket.emit('identify', { userId, role: 'hod' });
      const handleReconnect = () => socket.emit('identify', { userId, role: 'hod' });
  ...
  const menuItems = [
    { icon: LayoutDashboard, label: 'Overview', href: '/hod' },
    { icon: FilePlus, label: 'Create Exam', href: '/hod/generate' },
    { icon: FileText, label: 'Exams List', href: '/hod/exams' },
    { icon: Users, label: 'Students', href: '/hod/students' },
    { icon: Building2, label: 'Institution Flow', href: '/hod/institution-users' },
    { icon: PieChart, label: 'Analytics', href: '/hod/analytics' },
    { icon: FileText, label: 'Performance Reports', href: '/hod/performance-reports' },
  ];

  return (
    <div className="flex h-screen bg-[#F1F5F9] text-[#0F172A] font-sans selection:bg-[#F97316]/10 overflow-hidden">
      {/* Sidebar - Desktop */}
      <motion.aside
        initial={false}
      ...
      >
        <div className="h-20 flex items-center px-6 gap-3 border-b border-[#E2E8F0]">
          <div className="w-10 h-10 bg-[#F97316] rounded-xl flex items-center justify-center shrink-0 shadow-lg shadow-orange-500/20">
            <Sparkles className="text-white" size={24} />
          </div>
          {!collapsed && (
            <div className="flex flex-col">
              <span className="font-black text-lg tracking-tighter leading-none text-[#F97316]">HOD</span>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-20 bg-white border-b border-[#E2E8F0] flex items-center justify-between px-6 md:px-10 shrink-0 z-20">
          <div className="flex items-center gap-6 flex-1">
            ...
            <div className="md:hidden font-black text-lg text-[#F97316] truncate">HOD PORTAL</div>
          </div>

          <div className="flex items-center gap-2 sm:gap-4 shrink-0">
            <button className="h-10 w-10 sm:h-11 sm:w-11 flex items-center justify-center text-slate-400 hover:text-[#F97316] bg-slate-50 rounded-xl transition-all relative border border-slate-100 shrink-0">
              <Bell size={18} className="sm:w-5 sm:h-5" />
              <span className="absolute top-2 right-2 w-2 h-2 bg-orange-500 rounded-full border-2 border-white" />
            </button>
            <div className="hidden sm:block h-8 w-px bg-slate-100 mx-1" />
            <div className="hidden sm:flex flex-col items-end">
              <span className="text-sm font-bold text-slate-800">{user?.username || 'HOD User'}</span>
              <span className="text-[10px] uppercase font-black text-[#F97316] tracking-widest">HOD</span>
            </div>
            ...
            >
              <div className="h-20 flex items-center justify-between px-6 border-b border-[#E2E8F0]">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-black text-xl text-[#F97316]">HOD</span>
                </div>
