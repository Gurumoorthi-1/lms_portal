...
function StudentRow({ student, index }) {
  return (
    <motion.div
      ...
      className="group bg-white border border-[#E2E8F0] hover:border-[#7C3AED]/40 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all hover:shadow-md cursor-pointer"
    >
      <div className="flex items-center gap-4">
        <div className="w-12 h-12 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center text-[#7C3AED] font-black text-lg shadow-sm group-hover:bg-[#7C3AED] group-hover:text-white transition-all">
          {student.name.charAt(0)}
        </div>
        <div>
          <h3 className="font-bold text-slate-900 group-hover:text-[#7C3AED] transition-all flex items-center gap-2">
            {student.name}
      ...
      <div className="flex items-center gap-8 sm:gap-12 text-center sm:text-left">
        ...
        <button className="h-10 w-10 flex items-center justify-center text-slate-300 hover:text-[#7C3AED] transition-colors">
...
export default function StudentManagement() {
  ...
  if (loading) return <InstructorLayout><StudentsSkeleton /></InstructorLayout>;
      ...
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
             ...
             <button onClick={copyInviteLink} className="h-12 px-6 bg-[#7C3AED] text-white rounded-2xl font-bold hover:bg-[#6D28D9] transition-all shadow-lg shadow-purple-500/20 flex items-center gap-2 active:scale-95">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4">
           <div className="flex-1 w-full relative">
              ...
              <input 
                ...
                className="w-full h-14 pl-12 pr-4 bg-white border border-[#E2E8F0] rounded-2xl outline-none focus:border-[#7C3AED] transition-all text-sm font-medium text-slate-800 shadow-sm"
              />
           </div>
           <select 
             value={filterActive}
             onChange={(e) => setFilterActive(e.target.value)}
             className="h-14 px-4 bg-white border border-[#E2E8F0] rounded-2xl text-slate-500 font-bold text-sm w-full sm:w-auto outline-none focus:border-[#7C3AED] transition-all shadow-sm cursor-pointer hover:border-slate-300">
